...
class TestClass:
    ...
    def test_ini(self):
        a = _forest.RandomForest()
        a_t= a.test()
        t_b=_forest.b()
        c=DecisionTree("gini",3)
        assert t_b==1
        assert a_t==3
...
if __name__ == '__main__':
    ...
    c_train_num=x_train.shape[0]
    c_x_train = x_train.flatten()
    c_y_train = y_train.flatten()

    c_test_num=x_test.shape[0]
    c_x_test = x_test.flatten()
    c_y_test = y_test.flatten()
